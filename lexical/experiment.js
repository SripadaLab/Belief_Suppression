...
var getStim = function(practice) {
	...
	if (trial_type[0] == 'S') {
		...
		//generate a random nonword
		if (practice) {
			word = practice_nonwords.pop();
		} else {
			word = nonwords.pop();
		}
		if (trial_type[1] == 'T') {
	...
	if (trial_type[1] == 'T') {
		textprompt = '<div class = centerbox><p class="prompt ' + truthclass + '">Is this an English word?</div>';
	} else {
		textprompt = '<div class = centerbox><p class="prompt ' + lieclass + '">Is this an English word?</div>';
	}
	...
	var target = '<div class = "bigtext g1"></div>';
	var probe = '<div class = "bigtext g2">' + word + '</div>';
	var problem = '<div class = "bigtext g3"></div>';
	current_trial += 1;
	var stim = textprompt + '<div class="centerbox"><div class=container>' + target  + probe + problem + '</div></div>';
...
var instructions_block = {
	...
	pages: [
		'<div class = centerbox><p class = block-text>In this experiment you will see letters on the screen along with a prompt asking if the letters form a valid English word. </p><p class = block-text>If it is a real English word, press the <b>T</b> key. If it is not an English word, press the <b>F</b> key.</p><p class = block-text>Press <b>Enter</b> to start a short practice.</p></div>'
	],
	allow_keys: true,
...
var instructions_block_2 = {
	...
	pages: [
		'<div class = centerbox><p class = block-text>Now there will be one more instruction. When the prompt question text is in <b style="color: green">GREEN</b> text, your task is to give the answer you think is correct, just like you did in the prior trials. So you will press the <b>T</b> key if you are shown a real English word and the <b>F</b> key if it is not like you just did.</p><p class = block-text>But on some trials the prompt question text will be in <b style="color:red">RED</b>. This indicates that you should <b>lie</b> about whether the word is a real English word or not -- you should give the opposite of what you think the correct answer is. So if the text is red and the word is a real English word you would press the <b>F</b> key -- lying and indicating that it is not a word. If the text is red and the word is not a real English word you would press the <b>T</b> key -- lying and indicating that it is an English word.</p><p class = block-text>Press <b>Enter</b> to start a short practice.</p></div>'
	],
	allow_keys: true,
	show_clickable_nav: false,
	timing_post_trial: 1000,
